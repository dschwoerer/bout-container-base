...
# BOUT++ deps
RUN dnf -y install netcdf-devel netcdf-cxx4-devel hdf5-devel fftw-devel cmake \
                   python3-numpy python3-Cython python3-netcdf4 python3-scipy \
		   python3-boututils python3-boutdata python3-zoidberg python3-xbout \
		   flexiblas-devel gcc-c++ mpark-variant-devel python3-jinja2 \
		   petsc-$MPI-devel hdf5-$MPI-devel sundials-$MPI-devel sundials-devel \
		   git-core bison flex diffutils fakeroot python3-devel && dnf clean all
...
ENV MPI_BIN=/usr/lib64/$MPI/bin \
    ...
    MPI_PYTHON_SITEARCH=/usr/lib64/python3.13/site-packages/$MPI \
    MPI_PYTHON3_SITEARCH=/usr/lib64/python3.13/site-packages/$MPI \
...
RUN VER=$PETSC_VERSION && curl https://web.cels.anl.gov/projects/petsc/download/release-snapshots/petsc-$VER.tar.gz > petsc-$VER.tar.gz \
 ...
 && make all \
 && sudo make install \
 && sudo find /usr/local -name *.a -delete \
 && (test ".$TYPE" != ".mini" || sudo rm -rf /usr/local/share/petsc/examples ) \
 && cd .. && rm -rf .* * \
